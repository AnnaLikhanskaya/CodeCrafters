...
//        SearchService<RootVegetable> rootVegetableSearchService = new BinarySearchService<>();
    ...
    private static void handleCarMenu(Scanner scanner, FileService fileService, RandomService randomService,
                                      ManualService manualService, SortService<Car> sortService, Storage<Car> carStorage,
                                      SearchService<Car> carSearchService) {
        while (true) {
            ...
            switch (choice) {
                ...
                case 4:
                    if (rootVegetableStorage.getAll().size() > 0) {
                        ...
                        RootVegetable searchKey = new RootVegetable.Builder().setType(type).setWeight(0.1)
                                .setColor("Unknown").build();
